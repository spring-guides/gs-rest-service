...
* Use the `start.spring.io` service to bootstrap a project.
* Create a **Resource Representation Class** (Java Record or Kotlin Data Class).
* Build a **REST Controller** using `@RestController` and `@GetMapping`.
* Use `@RequestParam` to handle query parameters.
* Leverage Spring's automatic JSON conversion via Jackson.
